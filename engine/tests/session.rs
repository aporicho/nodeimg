...
impl NodeSource for SessionTestSource {
    fn collect(&self) -> Vec<NodeRegistration> {
        vec![NodeRegistration {
            ...
            capability_bindings: vec![("test.preview_value".into(), Arc::clone(&self.executor))],
        }]
    }
}
...
fn assert_float(value: Option<Value>, expected: f32) {
    assert!(
        matches!(value, Some(Value::Float(actual)) if (actual - expected).abs() < f32::EPSILON)
    );
}
...
async fn preview_execution_does_not_populate_formal_cache() {
    ...
    let ticket = session
        .request_export(ExecuteTarget::Node(node_id))
        .await
        .unwrap();
    ...
    let outputs = engine
        ...
        .unwrap();
    assert!(
        matches!(outputs.get("value"), Some(Value::Float(actual)) if (*actual - 1.0).abs() < f32::EPSILON)
    );
}
