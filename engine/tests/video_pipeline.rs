...
async fn test_headless_video_pipeline_writes_animated_output() {
    ...
    registry.register(sources::ApiVideoGenerationSource::with_provider_registry(
        std::sync::Arc::new(video_providers),
    ));
    ...
    let ticket = match ticket {
        ...
        ExecutionRequestResult::Queued { .. } => {
            panic!("expected video pipeline execution to start")
        }
    };
