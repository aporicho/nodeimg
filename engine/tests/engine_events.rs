...
async fn engine_subscription_receives_lifecycle_events() {
    ...
    let ticket = match ticket {
        ...
        ExecutionRequestResult::Queued { .. } => {
            panic!("expected immediate engine execution start")
        }
    };
...
async fn execute_request_returns_before_background_execution_finishes() {
    ...
    let ticket = match ticket {
        ExecutionRequestResult::Started(ticket) => ticket,
        ExecutionRequestResult::Queued { .. } => {
            panic!("expected slow execution to start immediately")
        }
    };
    ...
    let outputs = engine
        ...
        .unwrap();
    assert!(
        matches!(outputs.get("value"), Some(Value::Float(actual)) if (*actual - 2.0).abs() < f32::EPSILON)
    );
}
