...
impl Executor for ArtifactPassthroughExecutor {
    ...
    fn execute<'a>(
        ...
        Box::pin(async move {
            call_count.fetch_add(1, Ordering::SeqCst);
            let image = req.inputs.get("image").cloned().ok_or_else(|| {
                engine::execution::ExecutorError::InvalidInput {
...
async fn full_image_generation_persists_artifact_and_params_snapshot() {
    ...
    let mut engine = Engine::from_registry_bundle_with_resources(
        build_registry(
            Arc::clone(&image_call_count),
            Arc::clone(&passthrough_call_count),
        ),
        None,
    ...
    let selected = engine
        .query_selected_artifact(node_id, "image")
        .unwrap()
        .unwrap();
...
async fn full_image_generation_restores_selected_artifact_across_engine_instances() {
    ...
    assert_eq!(
        engine
            .query_artifact_history(node_id, "image")
            .unwrap()
            .len(),
        1
    );
