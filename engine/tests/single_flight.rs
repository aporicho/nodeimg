...
use engine::events::{CancellationReason, CancellationSubject, EngineEvent, ExecutionStatus, QueueReason};
use engine::execution::{EvaluationFidelity, ExecutionMode, ExecutionOutputs, ExecutorError, NodeExecutionRequest};
use engine::executors::{Executor, ExecutorFuture, LocalityProfile};
use engine::facade::{EngineFacade, EngineError, ExecutionRequest, ExecutionRequestResult};
        ...
        Box::pin(async move {
            ...
            Ok(match req.fidelity.clone() {
                ...
                EvaluationFidelity::Full => {
                    ExecutionOutputs::full(HashMap::from([("value".to_string(), Value::Float(value))]))
                }
            })
        })
    }
...
impl NodeSource for SingleFlightSource {
    fn collect(&self) -> Vec<NodeRegistration> {
        vec![NodeRegistration {
            ...
            capability_bindings: vec![("test.single_flight".into(), Arc::new(SingleFlightExecutor))],
        }]
    }
}
...
async fn full_request_is_rejected_while_full_is_running() {
    ...
    assert_eq!(engine.await_execution(started.execution_id).unwrap(), engine::execution::ExecutionTerminalStatus::Cancelled);
}
...
async fn full_request_queues_behind_preview() {
    let mut engine = make_engine();
    let node_id = engine.add_node("single_flight_value").unwrap();
    let preview = engine.execute_request(preview_request(node_id)).await.unwrap();
    ...
    let pending_id = match queued {
        ExecutionRequestResult::Started(_) => panic!("expected full request to queue behind preview"),
    ...
    for _ in 0..20 {
        ...
        if matches!(state.current, Some(ref current) if current.from_pending_id == Some(pending_id)) {
            ...
            engine.await_execution(state.current.unwrap().execution_id).unwrap();
            break;
        }
        std::thread::sleep(Duration::from_millis(10));
    }

    assert!(saw_resubmitted_start, "pending full should be resubmitted after preview completes");
}
...
async fn preview_preempts_running_full_and_keeps_full_pending() {
    ...
    let preview = match preview {
        ExecutionRequestResult::Started(ticket) => ticket,
        ExecutionRequestResult::Queued { .. } => panic!("expected preview to start after preempting full"),
    };

    let state = engine.query_state();
    assert!(matches!(state.current, Some(ref current) if current.execution_id == preview.execution_id));
    assert!(state.pending_full.is_some());

    let cancelled = engine.await_execution(full.execution_id).unwrap();
    assert_eq!(cancelled, engine::execution::ExecutionTerminalStatus::Cancelled);
