...
fn test_preview_does_not_affect_undo() {
    ...
    engine.set_param(id, "brightness", Value::Float(0.5), true); // preview
...
fn test_commit_clears_preview() {
    ...
    engine.set_param(id, "brightness", Value::Float(0.75), false);
...
fn test_undo_and_redo_clear_preview() {
    ...
    engine.set_param(id, "brightness", Value::Float(0.25), true);
    assert!(engine.graph_state_summary().has_preview);
