...
use engine::execution::{EvaluationFidelity, ExecutionMode, ExecutionOutputs, NodeExecutionRequest};
...
fn encode_video(path: &PathBuf, frames: &[DynamicImage], fps: u32) -> Result<(), engine::execution::ExecutorError> {
...
fn build_frame(width: u32, height: u32, prompt: &str, frame: u32) -> DynamicImage {
    ...
    let seed = prompt.bytes().fold(0_u32, |acc, byte| acc.wrapping_mul(33).wrapping_add(byte as u32));
...
async fn full_video_generation_persists_artifact_and_params_snapshot() {
    ...
    engine.set_param(node_id, "prompt", Value::String(String::from("ocean")), false);
    ...
    assert_eq!(history[0].params_snapshot.get("prompt"), Some(&String::from("ocean")));
    assert_eq!(history[0].params_snapshot.get("fps"), Some(&String::from("8")));
...
async fn full_video_generation_restores_selected_artifact_across_engines() {
    ...
    assert!(video_path.exists());
    assert_eq!(engine.query_artifact_history(node_id, "video").unwrap().len(), 1);
}
...
async fn selecting_video_artifact_syncs_params_and_restores_without_reexecution() {
    ...
    engine.set_param(node_id, "prompt", Value::String(String::from("forest")), false);
    ...
    assert_eq!(call_count.load(Ordering::SeqCst), 2);
    let outputs = engine.query_execution_outputs(third.execution_id, node_id).unwrap();
