...
use crate::execution::{
    EvaluationFidelity, ExecutionMode, ExecutorProgressEvent, PreviewPrecision, ProgressSink,
};
use crate::facade::{
...
impl ProgressSink for SessionPreviewSink {
    fn report(&self, event: ExecutorProgressEvent) {
        ...
        store
            .latest_preview
            .insert((node_id, output.clone()), value);
        drop(store);
...
impl Session {
            ...
            InteractionEvent::ParamChanged {
                ...
                if preview {
                    self.preview_overlay = Some(PreviewOverlay {
                        target: PreviewTarget { node_id, param },
                        value,
                    });
                    self.preview_store
                        .lock()
    ...
    pub async fn request_preview(
        &mut self,
        target: ExecuteTarget,
    ) -> Result<PreviewRequestStatus, SessionError> {
        let (preview_graph, cacheable_target) = {
            ...
            resolve_subgraph(&preview_graph, target.clone()).map_err(|error| {
                SessionError::InvalidInteraction(format!(
                    "failed to resolve preview target: {error}"
                ))
            })?;
            drop(engine);
            let scheduler = self
            ...
            let cacheable_target = match target {
                ExecuteTarget::Node(node_id) => Some((
                    node_id,
                    scheduler
                        .query_signature(&preview_graph, &target)?
                        .ok_or_else(|| {
                            SessionError::InvalidInteraction(format!(
    ...
    pub fn await_execution(
        &self,
        execution_id: u64,
    ) -> Result<ExecutionTerminalStatus, SessionError> {
        Ok(self
            .scheduler
            .lock()
            .expect("session scheduler lock poisoned")
    ...
    fn build_preview_graph(
        &self,
        base_graph: &crate::graph::Graph,
    ) -> Result<crate::graph::Graph, SessionError> {
    ...
    fn record_interaction_ack(&mut self) {
        ...
        let _ = self
            .events
            .publish(SessionEvent::InteractionAck { event_id });
    }

    fn record_undo_stack_changed(&mut self, can_undo: bool, can_redo: bool) {
        let _ = self
            .events
            .publish(SessionEvent::UndoStackChanged { can_undo, can_redo });
    }
}
