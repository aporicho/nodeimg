...
use crate::node::eval::Connection;
use crate::node::registry::{NodeDef, NodeInstance, NodeRegistry, ParamDef, PinDef};
use crate::node::types::{DataTypeId, DataTypeInfo, DataTypeRegistry, Value};
...
impl BackendClient {
    ...
    fn parse_widget_override(param: &serde_json::Value) -> Option<crate::node::widget::WidgetId> {
        let widget = param.get("widget")?.as_str()?;
        Some(crate::node::widget::WidgetId::new(widget))
    }
...
mod tests {
    ...
    fn test_serialize_ai_subgraph() {
        use crate::node::registry::{NodeDef, NodeInstance, PinDef};
        ...
        let connections = vec![crate::node::eval::Connection {
            from_node: 0,
            from_pin: "model".into(),
            to_node: 1,
            to_pin: "model".into(),
