...
- 28 个节点同时有 CPU（`process`）和 GPU（`gpu_process`）两套实现
...
| 文件 | 保留内容 | 理由 |
|------|---------|------|
| `color.rs` | `render_histogram_image()` | histogram 节点的 CPU 渲染，无 GPU 替代 |
...
改为：如果节点既没有 `process` 也没有可执行的 `gpu_process`（gpu_ctx 为 None），**返回错误**。

```rust
// 改前：静默跳过
// 改后：
if def.process.is_none() && (def.gpu_process.is_none() || gpu_ctx.is_none()) {
    if !is_ai_node { // AI 节点走后端，不在这里处理
        return Err(format!("Node '{}' has no executable path (GPU unavailable?)", def.title));
    }
}
```
...
- `crates/nodeimg-engine/src/eval.rs` — 修改执行逻辑，无路径时报错
