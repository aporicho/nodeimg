...
节点通过统一的 `execute` 函数定义处理逻辑，接收包含 CPU 和 GPU 能力的 `ExecContext`，可按任意顺序调用：
...
node! {
    ...
    execute: |ctx, inputs| {
        ctx.gpu(|gpu| gpu.run_shader(include_wgsl!("shader.wgsl"), &inputs))
    },
}
...
node! {
    ...
    execute: |ctx, inputs| {
        ...
        ctx.gpu(|gpu| gpu.run_shader(include_wgsl!("shader.wgsl"), &inputs, &lut))
    },
}
```
