...
flowchart TB
    ...
    是否AI -->|"AI/API + auto 模式"| 跳过["跳过，保持脏标记"]:::internal
    是否AI -->|"Image / manual 模式"| 分发{"按 executor 分发"}:::internal
...
- **执行规划器**：对比当前图版本和上次执行时的图版本，找出变化节点，沿拓扑向下游传播脏标记，按层分组输出执行计划。auto 模式下，AI 和 API 节点标脏但不加入执行计划（等待手动触发）。
...
- **自动执行跳过 AI/API 节点**：set_param 触发的自动执行中，规划器遇到 AI 或 API 节点时标脏但不加入执行计划，等待用户手动触发。GPU/Image 节点链实时刷新，AI/API 节点输出保持上次结果直到手动运行。
