...
flowchart TB
    ...
    验证 -->|"通过"| 快照["当前版本入 undo 栈"]:::internal
...
- **图编辑器**：add_node、remove_node、connect、disconnect、set_param、move_node。每次操作创建新版本，旧版本入 undo 栈。
...
- **不进入 undo 栈的操作**：move_node（画布坐标变化）可选择不记录，避免拖拽时产生大量版本。
