...
节点最终都要收敛为统一的 `NodeDef`（节点定义，NodeDef），进入 `NodeManager`（节点管理器，NodeManager）注册表。

### 设计决议

1. `NodeDef` 是唯一权威节点模型；
2. `NodeDef` 采用“瘦核心 + 默认化 `ExecutionPolicy`（执行策略，ExecutionPolicy）”结构；
3. 动态参数不进入静态 `NodeDef`，而由 `Provider.param_schema(...)`（参数结构查询，param_schema）提供；
4. `executor_type` 是调度与执行分发的权威来源，必须显式声明。
