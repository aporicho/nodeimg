...
- `Param.expose` 必须非空，且值只能是 `control` / `input` / `output`
- 复杂类型参数（`IMAGE / MASK / MODEL / CLIP / VAE / LATENT / CONDITIONING`）只能使用 `default=None`
- 输入侧接口名必须唯一，输出侧接口名必须唯一
- 允许同名输入与同名输出共存（如 `latent -> latent` 透传节点）
...
- `get_exposed_pin` / `get_exposed_input_pin` / `get_exposed_output_pin` 可作为按名称查询的便捷接口
- `Param.expose` 进入正式接口视图；它不再只是元数据备注

### 展开接口便捷查询

在 `list_exposed_pins` 之上，节点管理器还提供按名称查询的便捷接口：

- `get_exposed_pin(type_id, pin_name)`：按名称查询任意正式接口
- `get_exposed_input_pin(type_id, pin_name)`：按名称查询输入接口
- `get_exposed_output_pin(type_id, pin_name)`：按名称查询输出接口

这些接口属于 `list_exposed_pins` 的薄包装，不引入新的展开规则。
...
├── collect/
│   ├── mod.rs                      // 仅导出
│   ├── collect_inventory_defs.rs   // 从统一 inventory 收集全部节点定义
...
└── query/
    ...
    ├── get_exposed_pin.rs          // 按名称查询展开后的接口
    ├── get_exposed_input_pin.rs    // 按名称查询输入接口
    ├── get_exposed_output_pin.rs   // 按名称查询输出接口
...
| `collect/collect_inventory_defs.rs` | 从统一 inventory 收集全部节点定义 |
...
| `query/get_exposed_pin.rs` | 实现“按名称查询展开后的接口” |
| `query/get_exposed_input_pin.rs` | 实现“按名称查询输入接口” |
| `query/get_exposed_output_pin.rs` | 实现“按名称查询输出接口” |
...
- 基础查询：`get_node_def`、`list_node_defs`、`list_nodes_by_category`、`search_nodes`、`list_categories`
- Python 节点声明规格生成：`python_node_decl_specs`
- Python 节点编译期注册：`from_inventory()` 包含 Python 节点
- `Param.expose` 规范化解析
- 展开后的正式接口视图：
  - `list_exposed_pins`
  - `list_exposed_input_pins`
  - `list_exposed_output_pins`
  - `get_exposed_pin` / `get_exposed_input_pin` / `get_exposed_output_pin`
- Python 声明规则校验的正向保障：
  - `Param.expose` 非空
  - 复杂类型默认值限制
  - 输出引脚不带 `required`
