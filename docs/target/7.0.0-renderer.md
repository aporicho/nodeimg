...
**着色：** 光栅化后逐像素替换 RGB，保留原始 alpha（与 iced 相同方式）。不依赖 SVG 源文件中的颜色格式。
...
|------|------|------|------|
| wgpu | 27 | GPU 渲染 | 已有 |
| winit | 0.30+ | 窗口 / 事件循环 | 已有（通过 iced） |
| glyphon | 0.6+ | 文字整形 + 光栅化 | 已有（通过 iced） |
| resvg | 0.45 | SVG → 位图 | 已有（通过 iced） |
| lyon | 1.0 | 路径 tessellation | 已有（通过 iced） |
