...
└── renderer/
    ...
    ├── style.rs                      RectStyle、TextStyle、Border、Shadow、Stroke、Fill
    ├── path.rs                       PathData / PathStyle / PathRequest
    ├── vector_tessellator.rs         通用向量路径三角化 + 缓存
    ├── quad.rs                       圆角矩形管线（lyon tessellation）
    ├── vector.rs                     通用向量路径 GPU 管线
