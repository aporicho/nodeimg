...
> 调用 AI 运行时执行 AI 节点。当前实现为 Python HTTP 运行时，未来可替换为其他运行时。

## AIRuntime 接口

AI 执行器面向抽象接口编程，不绑定"Python over HTTP"：
...
// 当前实现
struct PythonHttpRuntime { base_url: String }

// 未来可能
// struct LocalRustMLRuntime { ... }
// struct RemoteCloudRuntime { ... }
```

引擎持有 `Arc<dyn AIRuntime>`，替换运行时只需替换注入的实现，AI 执行器代码不变。

### 边界说明

- `AIRuntime`（AI 运行时，AIRuntime）是 AI 执行器的下游依赖接口；
- AI 执行器本身仍应实现统一 `Executor`（执行器，Executor）接口；
- 调度器只依赖统一执行器接口，不直接依赖 `AIRuntime`。
...
- `request` 内含节点级取消令牌、超时、进度通道；
- AI 执行器将这些信息翻译给 `AIRuntime`；
- 是否写产物由运行时根据 `artifact_policy`（产物策略，artifact_policy）统一决定。
...
| 超时控制 | 从节点定义读取超时时长，收到 progress 重置计时器，超时触发 POST /node/cancel |
...
- **超时控制**：超时时长从 NodeDef 读取（Python 侧 `@node(timeout=600)`，默认 60s），不硬编码。收到 progress 事件重置计时器。超时触发时调用 `POST /node/cancel/{execution_id}` 中断 Python 端执行。
