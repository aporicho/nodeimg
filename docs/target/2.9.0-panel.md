...
│   ├── button.rs                  原子：Button
│   ├── slider.rs                  原子：Slider
│   ├── text_input.rs              原子：TextInput
│   ├── dropdown.rs                原子：Dropdown
│   ├── toggle.rs                  原子：Toggle
│   ├── image_viewer.rs            原子：ImageViewer
│   ├── text_display.rs            原子：TextDisplay
│   ├── list_view.rs               原子：ListView
│   ├── search_box.rs              原子：SearchBox
│   ├── group.rs                   原子：Group
│   ├── separator.rs               原子：Separator
│   ├── title_bar.rs               组合：TitleBar
│   ├── number_input.rs            组合：NumberInput
│   └── color_picker.rs            组合：ColorPicker
...
1. 创建 `gui/src/controls/my_control.rs`
2. 判断有无状态 → 函数或 struct
3. 定义暴露的事件接口
4. 遵循视觉规范
5. 在 `gui/src/controls/mod.rs` 中导出

### 新增组合控件

1. 创建 `gui/src/controls/my_composite.rs`
2. 用原子控件 + 布局节点组合
3. 定义暴露的事件接口（聚合内部事件）
4. 在 `gui/src/controls/mod.rs` 中导出
