...
除了八个组成部分，框架还需要提供以下基础设施，控件在其上运行：

### 动画系统（Animation）

状态变化时的平滑过渡。hover 背景色渐变、展开/收起动画、滚动惯性等都依赖它。

框架提供统一的插值引擎（起始值 → 目标值 → 持续时间 → 缓动函数），控件只需声明"从 A 过渡到 B"，不需要自己管帧计算。应该在框架设计早期考虑，后期加入会导致大量改造。

### 主题/样式（Theming）

控件的视觉属性（颜色、圆角、间距、字号）统一管理。

控件不硬编码颜色，而是读取主题 token（如 `theme.hover_bg`）。换肤 = 替换一套 token，所有控件自动更新。

### 文字输入 / IME（Input Method）

中日韩等语言的输入法支持。需要框架和系统 IME 双向通信：

- 框架 → IME：光标位置、编辑区域矩形（IME 据此定位候选窗）
- IME → 框架：预编辑文本（正在输入的拼音）、确认文本

这是桌面 GUI 框架中"基本但极其繁琐"的部分。

### 无障碍（Accessibility）

让屏幕阅读器等辅助技术理解控件。框架自动暴露控件的语义角色（按钮、滑块、文本框）和状态（值、禁用、选中），控件开发者不需要额外工作。

对创意工具类应用优先级较低，但框架应预留接口。

### 组合（Composition）

复合控件由原子控件嵌套组合而成（如 ColorPicker = Slider×3 + TextInput + Label）。框架的布局、事件、绘制系统都必须天然支持任意深度的嵌套，复合控件和原子控件在框架眼中没有区别。
