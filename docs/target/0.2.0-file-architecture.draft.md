...
│   └── topology.rs         拓扑查询器
│
├── registry/                                               ← 4.4.0 + 4.3.0
│   ├── mod.rs              NodeManager, NodeRegistry
│   ├── def.rs              NodeDef, PinDef, ParamDef
│   └── inventory.rs        inventory 收集
│
├── scheduler/                                              ← 4.1.0
│   ├── mod.rs              Scheduler
...
├── cache/                                                  ← 4.5.0
│   ├── mod.rs              Cache facade
│   ├── result.rs           结果缓存 (RwLock + LRU, Handle 豁免)
│   └── texture.rs          纹理缓存 (VRAM LRU)
│
├── artifact/                                               ← 4.6.0
│   ├── mod.rs              ArtifactManager
│   ├── storage.rs          产物存储 (文件 I/O)
│   ├── index.rs            产物索引 (artifacts.json)
│   ├── selection.rs        版本选用
│   └── handler.rs          ArtifactHandler trait + 注册
│
...
│   ├── file.rs             FilePicker
│   ├── text.rs             TextInput + TextArea          ← 新增，String 无约束/Multiline 映射
│   ├── floating.rs         FloatingPanel（悬浮面板容器）  ← 新增，工具栏/预览面板等悬浮层基础组件
...
| 4.3.0 | 节点(概念) | `engine/src/registry/def.rs` + `builtins/` + `api_nodes/` |
| 4.4.0 | 节点管理器 | `engine/src/registry/` |
