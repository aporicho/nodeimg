...
│
├── scheduler/                                              ← 4.1.0 (.impl)
│   ├── mod.rs              仅导出
│   ├── manager.rs          Scheduler 对外总入口
│   ├── model/
│   │   ├── mod.rs
│   │   ├── execution_mode.rs   Auto / Manual 模式
│   │   ├── execution_request.rs 执行请求模型
│   │   ├── execution_plan.rs   执行计划与分层结果
│   │   ├── dirty_state.rs      脏节点与脏原因
│   │   ├── runtime_state.rs    运行状态 / 最近一次执行状态
│   │   ├── state_summary.rs    调度器状态摘要
│   │   ├── event.rs            调度器事件模型
│   │   └── error.rs            错误定义
│   ├── planner/
│   │   ├── mod.rs
│   │   ├── snapshot_rebuild.rs 整图替换/恢复时的状态重建
│   │   ├── dirty_propagation.rs 脏传播
│   │   └── plan_builder.rs     执行计划构建
│   ├── runtime/
│   │   ├── mod.rs
│   │   ├── input_resolver.rs   输入解析与默认参数补全
│   │   ├── cache_restore.rs    缓存未命中后的产物回源
│   │   ├── executor_registry.rs 执行器注册与匹配
│   │   ├── node_runner.rs      节点执行分发
│   │   └── artifact_sink.rs    可持久化结果写入产物
│   └── cancel/
│       ├── mod.rs
│       ├── cancel_token.rs     取消标记
│       └── running_task.rs     运行中任务句柄
│
├── cache/                                                  ← 4.5.0
│   ├── mod.rs              Cache facade
