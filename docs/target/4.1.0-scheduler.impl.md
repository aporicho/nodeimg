...
### D27b：Auto 模式默认 throttle = 300ms

- Auto 模式下，图发生变化（参数调整、连线变动等）后，调度器等待 **300ms** 再触发执行，避免快速连续操作导致频繁重算；
- 默认开启；
- throttle 时长全局可配置，300ms 为首版默认值；
- throttle 期间若再次发生图变化，重置计时器（trailing-edge debounce）。
