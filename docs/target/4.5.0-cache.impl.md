...
缓存管理器对外只暴露缓存存取与失效接口，不暴露回源、调度或节点执行语义。
...
invalidate_texture(node_id, output_pin)
get_preview_texture(request)
clear_all()
stats()
```
...
- 缓存管理器不主动从产物管理器回源；
- 回源由调度器 / 运行时显式驱动；
- 节点级缓存资格来自 `NodeDef.execution.cache_policy`（缓存策略，cache_policy）。
...
> **水位配置决议**：`soft` / `hard` 水位以**百分比**为主要配置方式（相对系统可用 VRAM / RAM），同时支持绝对字节数覆盖；首版提供应用级全局配置，不支持按节点细粒度配置。
