...
在 targetv2 中,产物不是后台 cache 的同义词,而是用户可见工作流状态:Python/API 多轮生成候选、显式产物管理节点的候选列表、`save_image` / `save_video` 的导出结果都属于产物历史。
...
- **候选历史产物**：项目内版本化产物,可进入节点输出历史,支持浏览、标记、清理和当前版本选用。典型例子是 Python/API 生成节点输出。
- **导出历史产物**：`save_image` / `save_video` 等导出节点产生的正式结果,进入节点输出历史,可追溯和复用;默认不作为上游回源输入。
- **可回源产物**：当前选用版本可在缓存未命中时按规则回填缓存。候选历史产物通常可回源;导出历史产物是否回源由节点语义决定。

这里要强调：

- “**是否可回源**”是产物的**子类能力**，不是产物定义本身；
- 也就是说，**先是产物，再区分是否参与当前版本选用与回源**。
...
当前版本的产物管理器负责**用户可见产物历史**的统一管理。导出型产物进入导出历史;中途预览不进入正式候选历史。
...
| 选用 | 将某个历史版本设为当前选用，并通知调度器标脏下游节点 |
| 标记 | 给某个历史版本打标记,用于用户筛选和保留 |
...
| 能力 | 说明 |
|------|------|
| 导出 | 将已有产物导出到项目外部，可附带格式转换、命名规则和元数据打包；导出节点自身的结果进入导出历史 |
...
- **当前选用版本属于图语义**：切换选用后,拥有历史的节点本身不重跑,只标脏下游;selected artifact identity 进入下游执行签名；
- **导出历史** 是正式产物历史的一部分,但默认不参与当前版本回源；
...
产物管理器提供的是持久化历史输出与回源查询接口，不是执行器内部副作用接口。

已确认决议：

- 执行器只返回执行结果；
- 是否写产物由调度运行时根据 `NodeDef.execution.artifact_policy`（产物策略，artifact_policy）统一决定；
- 是否允许回源，也由调度运行时显式调用产物管理器。
...
flowchart TB
    ...
    运行时["执行运行时"]:::external -->|"Image 结果"| 接收["接收 Image + 元数据"]:::internal
...
flowchart TB
    ...
    切换 --> 推送事件["推送产物选用事件"]:::internal
    推送事件 --> 事件系统["事件系统"]:::external
    推送事件 --> 标脏["Scheduler 标脏下游"]:::internal
```

**关键:** 选用旧候选版本不重跑上游。Runtime 在下游求值需要该值时再通过回源流程加载当前选用版本。

## 显式产物管理节点(targetv2)

`artifact_manager` 是图里的正式节点,不是 GUI 侧隐式状态:

- 一次主要管理一个结果流。
- 接图片管理图片候选,接视频管理视频候选;节点语义通用,不按图片 / 视频拆成两个用户可见节点。
- 对下游输出当前选中版本。
- Python 底层工作流的候选历史主要通过显式 `artifact_manager` 暴露。
- API 黑盒任务节点可以原生自带历史面板,但与 `artifact_manager` 共享同一 Artifact / History 抽象。
...
- **磁盘空间**：产物可能积累很多，**首版清理策略 = 仅手动清理**；不实现自动淘汰和自动 GC，用户通过显式操作删除旧版本或孤立产物。自动淘汰策略推迟到后续版本迭代。
