...
- **渲染** — `tree/paint.rs` 遍历整棵树，根据 NodeKind 分发：Container 画 Decoration、Leaf 按 LeafKind 走对应渲染管线、Widget 递归展开后继续遍历 → `PaintTarget` 接收图元命令 → `renderer/` 执行 GPU 绘制。测试可用 `RecordingPaintTarget` 记录 `PaintOp`，不依赖真实 GPU。
...
| 层级 | 作用 | 典型示例 |
|------|------|---------|
| 图元 (Primitive) | 不可再分的渲染单元，可带 id 和 gestures 直接使用 | Text, Image, Icon, Circle, Line, Curve, Path, Grid, Connection, CustomPaint |
