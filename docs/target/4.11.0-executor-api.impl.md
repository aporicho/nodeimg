...
API 执行器应实现统一 `Executor`（执行器，Executor）接口，由调度器按 `executor_type`（执行器类型，executor_type）选择。
...
- API 执行器对上游暴露统一接口；
- 对下游再把 `request.node_def + request.inputs` 翻译给 `Provider`；
- 进度事件通过事件系统统一 `publish(event)`；
- 产物写入不在 API 执行器内部完成。
