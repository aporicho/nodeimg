...
- `inputs`：普通输入引脚提供的输入值
- `params`：参数最终值；其值在进入 `execute(...)` 前已由引擎计算完成
- 参数最终值的计算顺序为：
  1. 参数输入接口的外部连线值
  2. 本地控件值
  3. `default`
...
- 参数输出接口（`expose=["output"]`）的值不由 `execute(...)` 返回
- 普通输出由 `execute(...)` 返回字典提供
- 参数输出由引擎根据参数最终值自动补齐
...
当前阶段不把 `runtime`、`handles` 等更多运行时能力纳入 P0，避免提前固化过宽接口。
...
| 状态 | 说明 |
|------|------|
| 创建 | Rust spawn Python 子进程（**lazy 启动：首次执行 AI 节点时触发，不在 App 启动时提前 spawn**；此策略已在 [`architecture-invariants.md` §8 M2](architecture-invariants.md) 登记为冻结决议） |
