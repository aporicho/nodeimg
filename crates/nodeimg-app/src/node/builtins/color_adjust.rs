...
fn process(
    ...
    if let Some(Value::Image(img)) = inputs.get("image") {
        ...
        let result = crate::processing::color::color_adjust(img, b, c, s);
...
fn gpu_process(
    ...
    let p = Params {
        brightness: match params.get("brightness") {
            Some(Value::Float(v)) => *v,
            _ => 0.0,
        },
        contrast: match params.get("contrast") {
            Some(Value::Float(v)) => *v,
            _ => 0.0,
        },
    ...
    let pipeline = gpu.pipeline(
        "color_adjust",
        include_str!("../../gpu/shaders/color_adjust.wgsl"),
    );
    let bind_group =
