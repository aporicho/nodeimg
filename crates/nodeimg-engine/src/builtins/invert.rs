use nodeimg_gpu::pipeline::{create_io_bind_group, dispatch_compute};
...
fn process(
    ...
    if let Some(Value::Image(img)) = inputs.get("image") {
        let result = nodeimg_processing::color::invert(img);
...
fn gpu_process(
    ...
    let pipeline = gpu.pipeline("invert", nodeimg_gpu::shaders::INVERT);
